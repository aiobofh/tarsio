...
test(extract_testcase_shall_return_0_on_success) {
  (void)m;
  assert_eq(0, extract_testcase(NULL, NULL));
}

test(extract_testcase_shall_search_for_unit_test_names) {
  m.strstr.retval = (char*)0x1234;

  extract_testcase(NULL, (char*)0x1234);

  assert_eq(1, m.strstr.call_count);
  assert_eq(0, strcmp("test(", m.strstr.args.arg1));
}

test(extract_testcase_shall_search_for_module_test_names) {
...
test(extract_testcase_shall_search_for_en_parentheis_using_index_of) {
  m.strstr.func = strstr;
  m.strlen.func = strlen;

  extract_testcase(NULL, "test(a_name)");
...
test(extract_testcase_shall_return_negative_1_if_parentheis_was_not_found) {
  m.strstr.func = strstr;
  m.strlen.func = strlen;

  m.index_of.retval = NULL;

  assert_eq(-1, extract_testcase(NULL, "test(a_name)"));
}

test(extract_testcase_shall_not_append_test_case_if_name_not_found) {
  m.strstr.func = strstr;
  m.strlen.func = strlen;
...
test(extract_testcase_shall_append_test_case_if_name_not_found) {
  char teststring[14];
  ...
  extract_testcase(NULL, teststring);

  assert_eq(1, m.testcase_append.call_count);
}

test(extract_testcase_shall_call_append_test_correctly) {
  char teststring[14];
  strcpy(teststring, "test(a_name)");
  m.strstr.func = strstr;
  m.strlen.func = strlen;
  ...
  extract_testcase((testcase_list_t*)0x1234, teststring);
...
test(extract_testcase_shall_classify_unit_test_correctly) {
  char teststring[14];
  strcpy(teststring, "test(a_name)");
  m.strstr.func = strstr;
  m.strlen.func = strlen;
...
test(extract_testcase_shall_classify_module_test_correctly) {
  char teststring[24];
...
test(parse_shall_call_extract_testcase_if_end_of_line_is_found) {
  const char* teststring = "something\n";

  quick_setup;

  assert_eq(1, m.extract_testcase.call_count);
}

test(parse_shall_call_extract_testcase_if_code_block_start_is_found) {
  const char* teststring = "something{";
...
test(parse_shall_not_call_extract_testcase_if_eol_found_in_block_comment) {
  const char* teststring = "/* something\n";

  quick_setup;

  assert_eq(0, m.extract_testcase.call_count);
}

test(parse_shall_not_call_extract_testcase_if_eol_found_in_row_comment) {
  const char* teststring = "// something\n";
